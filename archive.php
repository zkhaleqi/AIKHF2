...
error_reporting(0);
$select = mysql_query("SELECT * FROM farticles WHERE month = '$month' ORDER BY id DESC LIMIT 0,1"); ?>
<?php $i = 0;  while($row = mysql_fetch_array($select)) { ?>
<?php $month = $row['month'] ?>
<?php $year = $row['year'] ?>
<head>
<title>آرشیف مطالب ماه <?php echo $row['month'];?></title>
<meta charset="utf-8">
...
<div id="container" class="container_12"><!--Start main container-->
  <section id="main">
    ...
    <?php  
		...
		$year =$_REQUEST['year'];
		error_reporting(0);
		$sql = "SELECT * FROM farticles WHERE month = '$month' AND year = '$year' LIMIT 0,1";
		$rs_result = mysql_query ($sql, $con); 
		while ($row = mysql_fetch_assoc($rs_result)) { 
		?>
    <h1>آرشیف مطالب ماه <?php echo $row['month'].' سال'.$row['year'];?> به ترتیب تاریخ نشر</h1>
    <?php }; ?>
      <?php
         include 'dbcon.php';
		$month =$_REQUEST['month'];
		...
		$select = mysql_query("SELECT * FROM farticles WHERE month = '$month' AND year = '$year' AND category !='Others' AND status='show' ORDER BY id DESC LIMIT $start_from, 10 "); ?>
		<?php $i = 0;  while($row = mysql_fetch_array($select)) { ?>
		<?php $month = $row['month'] ?>
		<?php $year = $row['year'] ?>
		<?php $date = $row['date'] ?>
      ...
      <div class="pagination" align="center">
        <!--Page Numbeers-->
	   <?php include 'pagination/archives-page.php';?>
       <!--Page Numbeers--> 
      </div>
    </div>
